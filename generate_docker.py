...
def generate_dockerfile(base_image_name: str) -> None:
    ...
    missing = []
    if not hasattr(ConfigurationCanopycover, 'transformer_name') or not ConfigurationCanopycover.transformer_name:
        missing.append("Transformer name")
    if not hasattr(ConfigurationCanopycover, 'author_name') or not ConfigurationCanopycover.author_name:
        missing.append("Author name")
    if not hasattr(ConfigurationCanopycover, 'author_email') or not ConfigurationCanopycover.author_email:
        ...
        with open(dockerfile_name, 'w') as out_file:
            for line in template:
                if line.startswith('LABEL maintainer='):
                    out_file.write("LABEL maintainer=\"{0} <{1}>\"\n".format(ConfigurationCanopycover.author_name, \
                                   ConfigurationCanopycover.author_email))
